...
const FEATURES = [
  {
    ...
    body: "Skip the line. Order ahead and pick up when ready.",
  },
  {
    icon: "fa-mobile-screen",
    title: "Order anywhere",
    body: "Works on your phone, laptop, or tablet — no app to install.",
  },
  {
    icon: "fa-receipt",
    title: "Real-time updates",
    body: "Live status from preparing to ready for pickup.",
  },
];
...
const TESTIMONIALS = [
  {
    ...
    quote:
      "Sobrang bilis mag-order, ready na agad pag-dating ko sa tindahan!",
    icon: "fa-user-graduate",
  },
  {
    name: "Jasper, Student",
    quote:
      "Ang sarap ng latte. Lagi akong nag-order online tuwing exam week.",
    icon: "fa-user-tie",
  },
  {
    name: "Carla, Office worker",
    quote: "I love that I can track my order in real time.",
    icon: "fa-user-nurse",
  },
];
...
const STEPS = [
  {
    ...
    body: "Hand-crafted espresso, brewed coffee, cold drinks, and pastries — updated by our team daily.",
  },
  {
    icon: "fa-bag-shopping",
    title: "Build your order",
    body: "Add to cart, customize, and pick dine-in or takeaway. Pay at the counter when you arrive.",
  },
  {
    icon: "fa-bell",
    title: "Get a heads-up",
    body: "We email you when your order is being prepared and again when it's ready for pickup.",
  },
];
...
export default function HomePage() {
  return (
    <main className="relative flex flex-1 flex-col overflow-x-hidden">
      ...
      <div
        className="cc-bean"
        style={{ width: 320, height: 200, top: -80, left: -60 }}
      />
      <div
        className="cc-bean animate-float"
        style={{
          width: 220,
          height: 140,
          top: 120,
      ...
      <section className="relative z-10 mx-auto grid w-full max-w-6xl flex-1 grid-cols-1 items-center gap-12 px-6 py-10 lg:grid-cols-2">
        <Reveal direction="right">
          ...
          <h1 className="font-display mt-5 text-5xl font-bold leading-[1.05] text-[var(--color-primary)] sm:text-6xl lg:text-7xl">
            <Reveal as="span" direction="up" delay={120}>
              <span className="block">A warm cup.</span>
            </Reveal>
            <Reveal as="span" direction="up" delay={260}>
              <span className="block bg-gradient-to-r from-[var(--color-accent)] to-amber-600 bg-clip-text text-transparent">
                Without the wait.
              </span>
            </Reveal>
          </h1>
          ...
          <div className="mt-10 grid grid-cols-3 gap-3 text-center">
            {STATS.map((s, i) => (
              <Reveal
                ...
                delay={680 + i * 90}
                direction="up"
                className="cc-card cc-card-hover p-3"
              >
                <i
                  className={`fa-solid ${s.icon} text-[var(--color-accent)]`}
                />
        ...
        <Reveal direction="left" delay={140}>
          <div className="relative h-[480px]">
            ...
            <div className="pointer-events-none absolute inset-0 rounded-[2.5rem] bg-gradient-to-tr from-[var(--color-primary)]/40 via-transparent to-transparent" />
            ...
            <div className="absolute bottom-5 left-5 right-5 flex items-center gap-3 rounded-2xl bg-white/95 px-4 py-3 shadow-lg backdrop-blur">
              ...
              <div className="min-w-0 flex-1">
                <p className="font-display truncate text-sm font-bold text-[var(--color-primary)]">
            ...
            <div className="animate-float absolute -right-4 -top-6 hidden h-32 w-32 overflow-hidden rounded-2xl shadow-xl ring-4 ring-white sm:block">
              <Image
                src={SIDE_IMG_1}
                alt="Espresso pour"
                fill
            ...
            <div
              className="animate-float absolute -bottom-6 -left-6 hidden h-28 w-28 overflow-hidden rounded-2xl shadow-xl ring-4 ring-white sm:block"
            ...
            <div className="animate-float absolute -left-2 top-6 rotate-[-4deg] rounded-2xl bg-white px-3 py-2 shadow-lg">
              <span className="font-display flex items-center gap-1 text-xs font-bold text-[var(--color-primary)]">
                <i className="fa-solid fa-star text-[var(--color-accent)]" />{" "}
                4.9 Rated
              </span>
            </div>
          </div>
      ...
      {/* Scroll cue */}
      <Reveal delay={900}>
        <div className="relative z-10 mx-auto mb-6 flex w-full max-w-6xl flex-col items-center px-6 text-center text-[var(--color-muted)]">
          <p className="text-xs uppercase tracking-widest">Scroll to explore</p>
          <i className="fa-solid fa-chevron-down mt-2 animate-bounce text-[var(--color-primary)]" />
        </div>
      </Reveal>
              ...
              >
                <Reveal direction={reverse ? "left" : "right"}>
                  <div className="[direction:ltr]">
                    <span className="grid h-12 w-12 place-items-center rounded-full bg-[var(--color-primary)] text-[var(--color-accent)] shadow-md">
                      <i className={`fa-solid ${step.icon} text-lg`} />
                    </span>
                    <p className="mt-3 text-xs uppercase tracking-widest text-[var(--color-accent)]">
                      Step {i + 1}
                    </p>
                    <h3 className="font-display mt-1 text-2xl font-bold text-[var(--color-primary)] sm:text-3xl">
                      {step.title}
                    </h3>
                ...
                <Reveal direction={reverse ? "right" : "left"} delay={120}>
                  <div className="relative h-56 [direction:ltr] sm:h-72">
                    ...
                    <div className="pointer-events-none absolute inset-0 rounded-3xl bg-gradient-to-tr from-[var(--color-primary)]/30 to-transparent" />
                  </div>
                </Reveal>
              </div>
            );
            ...
            >
              ...
              <figcaption className="mt-4 flex items-center gap-3 border-t border-[var(--color-line)] pt-3">
                ...
                <div>
                  ...
                  <div className="flex gap-0.5 text-xs text-[var(--color-accent)]">
                    {[...Array(5)].map((_, k) => (
                      <i key={k} className="fa-solid fa-star" />
                    ))}
                  </div>
                </div>
              </figcaption>
